...
interface UseAgentPickerWorkspaceOptions {
  items: AgentPickerComponentItem[];
  itemsById: Map<string, AgentPickerComponentItem>;
}

export function useAgentPickerWorkspace({ items, itemsById }: UseAgentPickerWorkspaceOptions) {
  const [studies, setStudies] = useState<AgentPickerStudy[]>([]);
  ...
  const [syncState, setSyncState] = useState<AgentPickerSyncState>("connecting");
  const [lastSavedAt, setLastSavedAt] = useState<string | null>(null);
  const deferredQuery = useDeferredValue(query);
  const syncStateRef = useRef<AgentPickerSyncState>("connecting");
  const remoteRevisionRef = useRef(0);
  const localDirtyRef = useRef(false);
  const pendingRemoteSyncRef = useRef(false);
  const pendingMutationCountRef = useRef(0);
  const studiesRef = useRef<AgentPickerStudy[]>([]);
  ...
  const updateStudyViewport = useCallback(
    (studyId: string, viewport: AgentPickerViewport) => {
      let patch: { viewport: AgentPickerViewport; x: number; y: number } | null = null;
      ...
      queueSceneMutation(() => updateSceneNode(studyId, patch as { viewport: AgentPickerViewport; x: number; y: number }));
    },
    [isReadOnly, queueSceneMutation],
  );
