...
import type { AgentPickerComponentItem } from "../lib/types";

interface AgentPickerSidebarProps {
  query: string;
...
function formatSourceKindLabel(value: AgentPickerComponentItem["sourceKind"]) {
  switch (value) {
    case "project":
      return "Project";
    case "page-import":
...
export default function AgentPickerSidebar({
  query,
  onQueryChange,
  items,
  totalStudies,
  onAddItem,
}: AgentPickerSidebarProps) {
  const categoryMap = new Map<string, AgentPickerComponentItem[]>();
  ...
  return (
    <aside className="flex min-h-[280px] w-full min-w-0 flex-col overflow-hidden rounded-[2rem] border border-[#e7e7e7] bg-[#fcfcfc] sm:min-h-[360px] lg:h-full lg:min-h-0">
      ...
      <div className="agent-picker-scrollbar flex-1 overflow-y-auto px-4 py-4">
              ...
              <div className="space-y-2 border-t border-[#f0f0f0] p-3">
                {categoryItems.map((item) => (
                  <button
                    ...
                    draggable
                    onDragStart={(event) => {
                      event.dataTransfer.setData("text/agent-picker-item", item.id);
