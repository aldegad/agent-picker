...
import AgentPickerPreview from "./AgentPickerPreview";
import {
  clampCanvasPosition,
  agentPickerViewports,
  agentPickerViewportList,
  type AgentPickerComponentItem,
  type AgentPickerStudy,
  type AgentPickerViewport,
} from "../lib/types";

interface AgentPickerCanvasNodeProps {
  study: AgentPickerStudy;
...
export default function AgentPickerCanvasNode({
  study,
  item,
  zoom,
  readOnly,
  ...
  onViewportChange,
  onRemove,
}: AgentPickerCanvasNodeProps) {
  const config = agentPickerViewports[study.viewport];
  const availableViewports = agentPickerViewportList;
  const isLocked = Boolean(study.locked);
  const viewportIcons = {
    mobile: Smartphone,
    desktop: Monitor,
    original: Component,
  } satisfies Record<AgentPickerViewport, typeof Smartphone>;
    ...
    >
      ...
      >
        ...
        <div className="h-full w-full">
          <AgentPickerPreview item={item} viewport={study.viewport} propsPatch={study.propsPatch} />
        </div>
      </div>
    </article>
  );
