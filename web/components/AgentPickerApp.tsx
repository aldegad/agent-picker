...
import AgentPickerBoard from "./AgentPickerBoard";
import AgentPickerSidebar from "./AgentPickerSidebar";
import { useAgentPickerWorkspace } from "../lib/use-agent-picker-workspace";
import type { AgentPickerComponentItem } from "../lib/types";

interface AgentPickerAppProps {
  items: AgentPickerComponentItem[];
  itemsById: Map<string, AgentPickerComponentItem>;
}

export default function AgentPickerApp({ items, itemsById }: AgentPickerAppProps) {
  const workspace = useAgentPickerWorkspace({
    items,
    itemsById,
  });
