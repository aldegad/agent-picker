...
import AgentPickerCanvasNode from "./AgentPickerCanvasNode";
...
import {
  ...
  type AgentPickerSyncState,
  type AgentPickerViewport,
} from "../lib/types";

interface AgentPickerBoardProps {
  readOnly: boolean;
  studies: AgentPickerStudy[];
  selectedStudyId: string | null;
  zoom: number;
  copyState: "idle" | "copied";
  syncState: AgentPickerSyncState;
  lastSavedAt: string | null;
  getItem: (itemId: string) => AgentPickerComponentItem | undefined;
  ...
  onViewportChange: (studyId: string, viewport: AgentPickerViewport) => void;
...
export default function AgentPickerBoard({
  readOnly,
  studies,
  selectedStudyId,
  zoom,
  ...
  const syncTimestamp = lastSavedAt
    ...
    : null;
  const stageStyle: CSSProperties & Record<"--agent-picker-zoom", string> = {
    "--agent-picker-zoom": String(zoom),
  ...
  return (
    <section className="relative flex min-h-[420px] min-w-0 overflow-hidden rounded-[2rem] border border-[#e7e7e7] bg-[#f6f6f6] sm:min-h-[520px] lg:h-full lg:min-h-0">
      <div className="flex flex-1 overflow-hidden bg-[#f6f6f6]">
        <div ref={scrollContainerRef} className="agent-picker-scrollbar relative h-full w-full overflow-auto">
              ...
              <div
                ...
                onDrop={
                  readOnly
                    ...
                    : (event) => {
                        event.preventDefault();
                        const itemId = event.dataTransfer.getData("text/agent-picker-item");
                ...
                className="agent-picker-stage relative"
                style={stageStyle}
              >
                {studies.length === 0 ? (
