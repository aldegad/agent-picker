...
- In a host project that vendors Agent Picker, run the same command from the host root before inspecting anything else.
...
- Post progress with `pnpm run agent-pickerd:set-agent-note -- --author gemini --status acknowledged --message "Read the selection and investigating."`
